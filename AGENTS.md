...
* **BLE link** (`-DFREEINK_CAP_BLE_TRANSFER=1`, `src/network/BleLink.*`): always on while awake. Protocol in
  `docs/ble-transfer-protocol.md`; update that document when you change a control op, a transfer kind or a status field.
* **Firmware updates** are a file drop: `/firmware/firmware.bin` + `firmware.bin.sha256` (+ optional `firmware.bin.version`)
  (`src/network/FirmwareStaging.*`), verified by `FirmwareWatcher`, offered by `FirmwareReadyActivity`, installed with
  `SdFirmwareUpdateActivity`. `src/network/BuildStamp.h` is generated by `scripts/build_stamp.py`; do not edit it.
* Partitions come from `partitions.csv`: `otadata` 0xe000, `app0` 0x10000 and `app1` 0x650000 (6.25MB each), `spiffs`,
  `coredump`. SPIFFS is not mounted; persistent state lives on the SD card.
