...
* Env: `pio run -e x4pro` (also `x4pro-gh_release`, `x4pro-gh_release_rc`). Board `esp32-s3-devkitc1-n16r8`:
  ESP32-S3, 16MB flash, 8MB PSRAM (`-DBOARD_HAS_PSRAM`, `memory_type = dio_opi`). Device flag `-DFREEINK_DEVICE_X4PRO=1`.
* Hardware: 800x480 panel used in native landscape (SSD1677, UC8179 or UC8279, detected at boot), touchscreen,
  frontlight, Power button plus two side keys, native SDMMC SD card (`-DUSE_BLOCK_DEVICE_INTERFACE=1`), USB mass
  storage (`-DFREEINK_CAP_USB_MSC=1`).
* **No network stack.** It uses `build_flags_nonet` / `lib_deps_nonet` (`FREEINK_CAP_NETWORK=0`): no Wi-Fi, web
  server, WebDAV, OPDS, KOReader sync, HTTP OTA or TLS. Code behind `#if FREEINK_CAP_NETWORK` does not compile here.
* **BLE link** (`-DFREEINK_CAP_BLE_TRANSFER=1`, `src/network/BleLink.*`): always on while awake. Protocol version 2 in
  `docs/ble-transfer-protocol.md`, design in `docs/security-v2.md`; update the protocol document when you change a
  control op, a transfer kind or a status field.
* **BLE security:** LE Secure Connections only (`-DMYNEWT_VAL_BLE_SM_SC_ONLY=1`), bonding + MITM, passkey shown by
  `BlePairingActivity`; one connection (`-DMYNEWT_VAL_BLE_MAX_CONNECTIONS=1`). New bonds only inside the pairing
  window. The trusted host (`pair` / `hello` HMAC secret) lives in NVS namespace `bleauth` (`src/BleTrustedHostStore.*`),
  not on the SD card. Nothing in `status` beyond the pre-auth fields before authentication.
* **Firmware updates** are a file drop: `/firmware/firmware.bin` + `firmware.bin.sha256` + `firmware.bin.version` +
  `firmware.bin.sig`, all required (`src/network/FirmwareStaging.*`). `FirmwareWatcher` offers an image only when the hash
  matches, the version is newer than `X4_BUILD_STAMP` and the ECDSA P-256 signature verifies
  (`src/network/FirmwareSignature.*`); `FirmwareReadyActivity` offers it, `SdFirmwareUpdateActivity` re-verifies and
  installs it. The public key is `src/network/FirmwareSigningKey.h`; sign with
  `FIRMWARE_SIGNING_KEY_FILE=... scripts/make_firmware_json.sh`. **Never commit a private signing key** (releases read
  it from the `FIRMWARE_SIGNING_KEY` Actions secret). `src/network/BuildStamp.h` is generated by
  `scripts/build_stamp.py`; do not edit it.
* Partitions come from `partitions.csv`: `otadata` 0xe000, `app0` 0x10000 and `app1` 0x650000 (6.25MB each), `spiffs`,
  `coredump`. SPIFFS is not mounted; persistent state lives on the SD card, except BLE bonds and the trusted host,
  which are in NVS.
* Board checks: `CrossPointSettings::usesPowerGestures()` (power tap/hold/double tap, side-key holds for Back/Select,
  no on-screen Back) and `BoardConfig::isX4Pro()`.
* The RAM rules below were written for the C3. They are still the house style and still binding for code shared with
  the C3 boards, but PSRAM makes the X4 Pro far less tight; do not use it as the reason to skip them in shared code.
...
* MCUs: ESP32-C3 (single-core RISC-V @ 160MHz: X4, X3) and ESP32-S3 (dual-core Xtensa LX7: X4 Pro, X4 Classic, Sticky, Paper Mono and others)
* RAM: ~380KB usable on ESP32-C3 (VERY LIMITED - primary project constraint)
  * **NO PSRAM on C3**. The X4 Pro (S3) has 8MB PSRAM.
...
* **Environments** (in `platformio.ini`):
  ...
  * `x4pro`: Xteink X4 Pro development build (LOG_LEVEL=2, serial enabled, BLE link, no network stack)
  * `x4pro-gh_release` / `x4pro-gh_release_rc`: X4 Pro release and release-candidate builds
  * Other boards have their own envs (`sticky`, `x4c`, `papermono`, each with release variants); see `platformio.ini`
