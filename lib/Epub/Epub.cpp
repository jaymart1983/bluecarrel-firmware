...
std::string Epub::getCoverBmpPath(bool cropped, bool oneBit) const {
  // The filename carries a PIPELINE VERSION, and it has to be bumped whenever
  // the conversion changes -- the scaler, the dither, the bit depth, the tone
  // curve.
  //
  // generateCoverBmp() returns early if the file exists, so without this a
  // cover generated by an older build is reused forever and a fix to the
  // conversion appears to do nothing. That is not a hypothetical: a resampling
  // fix was measured as "marginally better" when what was actually on screen
  // was still the image the broken scaler had produced, because the only thing
  // that had changed was the code that would have regenerated it.
  //
  // Stale versions are left on the card rather than swept: they are a few tens
  // of KB per book, and deleting files this function only knows how to name is
  // a bigger risk than the space it recovers.
  constexpr int COVER_PIPELINE_VERSION = 6;
  const auto coverFileName = std::string("cover") + (cropped ? "_crop" : "") + (oneBit ? "_1b" : "") + "_v" +
                             std::to_string(COVER_PIPELINE_VERSION);
  return cachePath + "/" + coverFileName + ".bmp";
}

bool Epub::generateCoverBmp(bool cropped, bool oneBit) const {
  // Already generated, return true
  if (Storage.exists(getCoverBmpPath(cropped, oneBit).c_str())) {
    return true;
  }
  ...
  if (FsHelpers::hasJpgExtension(coverImageHref)) {
    ...
    HalFile coverBmp;
    if (!Storage.openFileForWrite("EBP", getCoverBmpPath(cropped, oneBit), coverBmp)) {
      return false;
    }
    const bool success = oneBit ? JpegToBmpConverter::jpegFileTo1BitBmpStream(coverJpg, coverBmp, cropped)
    ...
    if (!success) {
      ...
      Storage.remove(getCoverBmpPath(cropped, oneBit).c_str());
    }
  ...
  if (FsHelpers::hasPngExtension(coverImageHref)) {
    ...
    const bool success = oneBit ? PngToBmpConverter::pngFileTo1BitBmpStream(coverPng, coverBmp, cropped)
